...
4. How do I retrieve my Api Keys?
   - Users who wish to retrieve their api keys may reference the respective services' guide on how to retrieve your api keys.
...
def bind_keys(window, canvas):
    """Bind keys for scrolling the canvas."""
...
# Bind mouse wheel scrolling
    window.bind_all('<MouseWheel>', lambda event: canvas.yview_scroll(int(-1*(event.delta/120)), "units"))  # Windows

def show_faq():
    """Display the FAQ content in a new window."""
...
def show_tutorial():
    """Display the tutorial content in a new window."""
...
def clone_repository(progress_var):
    """Clone the GitHub repository to a selected directory."""
...
def progress_callback(progress_var):
    """Update the progress bar during repository cloning."""
...
def clone_repository_thread():
    """Run the clone repository function in a separate thread."""
